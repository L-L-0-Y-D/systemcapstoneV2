...
?>
 <?php 
        if(isset($_GET['id']))
        {
            ...
            $result_count = mysqli_query($con, "SELECT COUNT(*) as total_records FROM managetable WHERE status = 2 AND businessid = '$id'") or die(mysqli_error($con));
            //total records
            ...
            //query string
            $table_query = "SELECT * FROM managetable WHERE status = 2 AND businessid = '$id' ORDER BY tableid DESC LIMIT $offset, $total_records_per_page";
            // result
            $result = mysqli_query($con,$table_query) or die(mysqli_error($con));
?>
    <div class="container-fluid pt-3">
            <h4 class="text-dark"><?= $_SESSION['auth_user']['business_name'];?>'s Blockdate Archive</h4>
                                    ...
                                    if($item['businessid'] == $_SESSION['auth_user']['businessid'])
                                       ...
                                       <tr>
                                            ...
                                            <td>
                                                ...
                                                <button class="btn btn-success btn-sm" type="submit" value = "<?= $item['tableid']; ?>" name="restore_table_btn"><i class="fas fa-archive"></i> </button>
                                            </td>
                                       </tr>
                                       <?php
                                        }
